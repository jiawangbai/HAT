...
parser.add_argument('--adv-eps', default=2.0/255, type=float,
                    help='adversarial strength for adversarial perturbations')
parser.add_argument('--adv-lr', default=1.0/255, type=float,
                    help='learning rate of PGD')
parser.add_argument('--adv-kl-weight', default=0.01, type=float,
                    help='weight of KL-divergence for adversarial training')
parser.add_argument('--adv-ce-weight', default=3.0, type=float,
                    help='weight of ce-loss for adversarial training')
...
def train_one_epoch(epoch,
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if args.token_label:
            output_cle, output_aux, _ = model(input)
            output_cle = output_cle.detach()
        else:
            output_cle = model(input).detach()
        ...
        for a_iter in range(args.adv_iters):
            ...
            with amp_autocast():
                ...
                if args.token_label:
                    ...
                    output_adv = model(input + delta)
                    loss_ce = loss_fn(output_adv, target)
