...
from loss_adv import SoftTargetCrossEntropy, DistillationLoss
...
# Adversarial
parser.add_argument('--adv-epochs', default=200, type=int, metavar='N',
                    help='number of epochs for performing adversarial training')
parser.add_argument('--adv-iters', default=3, type=int, metavar='N',
                    help='number of iterations for adversarial augmentation')
parser.add_argument('--adv-eps', default=2.0/255, type=float,
                    help='adversarial strength for adversarial augmentation')
parser.add_argument('--adv-lr', default=1.0/255, type=float,
                    help='learning rate for adversarial augmentation')
parser.add_argument('--adv-kl-weight', default=0.01, type=float,
                    help='weight of KL-divergence for adversarial augmentation')
parser.add_argument('--adv-ce-weight', default=3.0, type=float,
                    help='weight of ce-loss for adversarial augmentation')
...
def main():
    ...
    # setup loss function
    if mixup_active:
        train_loss_fn = SoftTargetCrossEntropy(reduce=True)
    elif args.smoothing:
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        for a_iter in range(args.adv_iters):
            ...
            with amp_autocast():
                ...
                loss_kl = (F.kl_div(output_adv_logprob, output_cle_prob, reduce=False).sum(dim=1) +
                           F.kl_div(output_cle_logprob, output_adv_prob, reduce=False).sum(dim=1)).mean() / 2
                ...
                # loss = loss.mean()
                # print(loss.item(), loss_ce.item(), loss_kl.item())
...
def validate(model,
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if args.rank == 0 and (last_batch or
